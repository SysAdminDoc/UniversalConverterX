...
<#
.SYNOPSIS
  Freeze the texturekit sidecar into a single-file Windows exe.
#>
[CmdletBinding()]
param([switch]$Clean)
...
& $python -m pip install --quiet Pillow imageio
...
$size = (Get-Item (Join-Path $here 'texturekit.exe')).Length / 1MB
Write-Host ("[done] texturekit.exe ({0:N1} MB)" -f $size) -ForegroundColor Green
