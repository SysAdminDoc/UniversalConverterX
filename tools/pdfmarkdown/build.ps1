...
<#
.SYNOPSIS
  Freeze the pdfmarkdown sidecar into a single-file Windows executable.
#>
[CmdletBinding()]
param([switch]$Clean)
...
$size = (Get-Item (Join-Path $here 'pdfmarkdown.exe')).Length / 1MB
Write-Host ("[done] pdfmarkdown.exe ({0:N1} MB)" -f $size) -ForegroundColor Green
