...
- Fixed CLI tools-path handling by expanding defined environment variables, rejecting unresolved `%...%` and `$env:` syntax, and preventing literal variable-name directories during tool downloads or configuration updates.
...
- Fixed Downloader output selection to honor the configured default directory, report the effective Downloads/TEMP fallback in the Activity header, and refuse to start when no writable destination exists.
...
- Fixed PowerShell compression argument binding for explicit zero CRF/target values and made NDJSON event parsing safe under strict mode when optional fields are absent.
- Fixed Recorder and Downloader output-folder actions to open the directory itself instead of selecting it in its parent.
...
- Fixed Explorer preset menu latency with a process-static, directory-stamped, bounded preset cache and Clone reuse of the already-built submenu commands.
- Fixed Explorer selection isolation by storing each command's immutable path snapshot on its own instance instead of a cross-command static list.
- Fixed REST admission control with configured concurrent-job and queue-depth limits, 429 overload responses, and per-job containment shares that keep aggregate process and memory budgets bounded.
- Fixed the REST accept loop's cancellation path by stopping the listener once instead of allocating an unbounded infinite-delay task and token registration per request.
...
- Batch rename is transactional. The apply step performed sequential `File.Move` calls behind a comment claiming a two-pass algorithm, so renaming A→B while B→A failed outright and any mid-run failure left the set half-renamed with no way back. A plan is now built before anything moves: swaps and longer cycles get a collision-safe staging phase, dependency chains are ordered, and a collision with an untouched file or two rows targeting the same name is refused up front. A failure part-way through rolls the whole set back. An atomic journal backs a one-click Undo and is offered on next launch if a crash interrupted a run.
